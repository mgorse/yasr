...
static Tts_synth synth[] = {
    ...
    { "", "%s[:syn]", "\003", "[:sa le]", "[:sa c]", 0, 
      "[]{}\\|_@#^*<>\"`~", "" },                             /* DECtalk */
    { "s\r", "q {%s}\rd\r", "s\r", "l %c\r", NULL, 0, 
...
tts_saychar(unsigned char ch)
{
    ...
    if (!synth[tts.synth].charoff) {    /* assume on string does everything */
    ...
    for (i = 0; i < NUMOPTS; i++) {
        if ((opt[i].type & 0x3f) != 3 && 
            !opt[i].ptr && opt[i].synth == tts.synth) {
            stack[j++] = i;
...
void 
tts_sayphonetic(unsigned char ch) {
    if (!isalpha(ch)) { 
        tts_saychar(ch); 
        return; 
    }
