...
child()
{
    ...
    if (!geteuid()) {     /* if we're setuid root */
...
static char *
gulp(unsigned char *buf, int *size, unsigned char *cp, unsigned char **ep)
{
    int os = *size;
    int n;
...
kbsay()
{
    ...
    if (okbuflen < 99 && kbuf[0] > 32 && kbuf[0] < 128) {
...
static void 
win_csi(char *buf, unsigned char **pp, int *size)
{
    ...
    while (!*p || isdigit((int) *p) || *p == ';') {
        if (!*p) { 
            if (!(p = gulp((unsigned char *) buf, size, 
                           (unsigned char *) p, pp))) {
                return;
            }
        }
    ...
    switch (*p) {
        ...
        case 'L':     /* insert rows */
            ...
            for (i = win_scrollmax; i >= win->cr + x; i--) {
                (void) memcpy(win->row[i], win->row[i - x], 
                              win->cols * CHARSIZE);
            }
        ...
        case 'r':
            ...
            win_scrollmax = parm[1] ? MIN(parm[1], win->rows) - 1
                                    : win->rows - 1;
            break;
...
#endif
                if (ch == kbuf[0] && win->cr == oldcr && 
                    win->cc == oldcc && kbuflen) {

/* this character was (probably) echoed as a result of a keystroke */
                    kbsay();
                    win_addchr(ch, 0);
                    (void) memmove(kbuf + sizeof(int), kbuf, 
                                   (--kbuflen) * sizeof(int));
                } else {
    ...
    for (i = 0; i < len; i++) {
        if (ibuf[i] == lc && ++nc == ui.minrc - 1) {
            ...
            tts_out((unsigned char *) obuf, 
                    sprintf(obuf, "repeats %d times\r", nc + 1));
            olen = nc = 0;
        } else {
            obuf[olen++] = ibuf[i];
            if (ibuf[i] != lc) {
