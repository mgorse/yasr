...
void 
opt_add(void *ptr, int tree, char *name, int type, ...)
{
    Opt *optr;
    va_list args;
    int i;

    if (!strcasecmp(name, "synthesizer")) synthopt = curopt;
    optr = &opt[curopt];
    optr->ptr = ptr;
    optr->tree = tree;
    optr->name = name;
...
opt_getval(int num, int flag)
{
    ...
    if ((opt[num].type & OT_BITSLICE) && !flag) {       /* bitslice */
        while (v) {
...
static void 
opt_synth_update(int num, int optval)
{
    char *p1, *p2;
...
void 
opt_write_single(FILE *fp, int i)
{
    switch (opt[i].type & 0x3f) {
        case 0: 
        ...
        case 1: 
            (void) fprintf(fp, "%s=%s\n", opt[i].name, 
                    strro(opt[i].arg[opt_getval(i, 0)], ':', 0)); 
            break;
        case 2: 
...
opt_init()
{
    ...
    opt_add(&ui.curtrack, 0, "cursor tracking", OT_ENUM, 3, "off", 
            "arrow keys", "full");
    opt_add(&tts.synth, 0, "synthesizer", OT_ENUM, 6, "speakout", "dectalk", 
            "Emacspeak server", "doubletalk", "bns", "apollo");
 ...
 */
    ...
    opt_add(&rev.udmode, 0, "up and down arrows", OT_ENUM, 3, "speak line", 
            "speak character", "speak word");
...
/* Speakout settings (first index is 9) */
    ...
    opt_add(NULL, -2, "textual", OT_ENUM|OT_SYNTH|OT_BITSLICE, 2, 
            "off", "on:!,.:;", 0, "\005y%x", 3);
    opt_add(NULL, -2, "math", OT_ENUM|OT_SYNTH|OT_BITSLICE, 2, 
            "off", "on:%^*()/-<=>+", 0, "\005y%x", 2);
    opt_add(NULL, -2, "miscelaneous", OT_ENUM|OT_SYNTH|OT_BITSLICE, 2, 
            "off", "on:[]{}\\|_\"'@#$&", 0, "\005y%x", 1);
    opt_add(NULL, -2, "spaces", OT_ENUM|OT_SYNTH|OT_BITSLICE, 2, 
            "off", "on: ", 0, "\005y%x", 0);
  
/* DEC-Talk settings (first index is 18) */
    opt_add((void *) 4, -1, "rate", OT_VAL|OT_SYNTH, 75, 650, 1, "[:ra%d]");
    opt_add((void *) 8, -1, "volume", OT_VAL|OT_SYNTH, 0, 99, 1, 
            "[:vol set %d]");
    opt_add((void *)12, -1, "voice", OT_ENUM|OT_SYNTH, 10, "paul", "harry", 
            "frank", "dennis", "betty", "ursula", "rita", "wendy", "kit", 
            "val", 1, "[:n\\p]");
    opt_add((void *) 0, -1, "punctuation", OT_ENUM|OT_SYNTH, 3, 
...
/* DoubleTalk settings (first index is 23) */
    ...
    opt_add((void *) 20, -1, "voice", OT_ENUM|OT_SYNTH, 8, "paul", "vader", 
            "bob", "pete", "randy", "biff", "skip", "roborobert", 3, "\001%dO");
    opt_add((void *) 0, -1, "punctuation", OT_ENUM|OT_SYNTH, 4, 
            "none:\0017b", "some:\0016b", "most:\0015b", "all:\0014b", 3, NULL);
...
/* Braille 'n Speak settings (first index is 29) */
    ...
    opt_add((void *) 0, -1, "punctuation", OT_ENUM|OT_SYNTH, 4, 
            "none", "some", "most", "all", 4, "\005\\?");

/* Apollo settings (first index is 33) */
    opt_add((void *) 0, -1, "punctuation", OT_ENUM|OT_SYNTH, 2, 
            "off", "on", 5, "@P%d");
    ...
    opt_add((void *) 20, -1, "sentence pause", OT_VAL|OT_SYNTH, 1, 
            15, 5, "@D%x");
